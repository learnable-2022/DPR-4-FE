...
const handleLogin = async (e) =>{
  ...
  if(state.state.value === "doctor"){
    try{
      ...
      const userdetails={ token: response?.data.token ,
        email:response?.data.doctor };
        const userStringify = JSON.stringify(userdetails);
        localStorage.setItem("userdetails", userStringify);
  ...
  }else if(state.state.value === "patient"){
    try{
      const response = await axioscall.post(
        PATIENT_LOGIN,
                         ...
                         type="password"
                          ...
                          <div className="diverge">
                           <div>
                            ...
                            <div>
                            <Link to="/forgotPassword" className="linky"><span>Forgot password?</span></Link>
                            </div>
                          </div>
                          {isLoading ? <button>loading...</button>: <button className="btn"  type="submit" disabled={!userEmail ||!userPassword}>
                          Log in
                        </button>}
